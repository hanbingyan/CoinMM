    ...
    for sess in range(n_instance):
        ...
        for i_episode in range(n_periods + 1):
            ...
            if i_episode % 100 == 0:
                ...
                epiact_hist.append(action.copy())
        ...
        act_hist.append(epiact_hist)
        reward_hist.append(epireward_hist)
        order_hist.append(epiorder_hist)

    return Q_last, Q_hist, inventory_hist, act_hist, reward_hist, order_hist


if __name__ == '__main__':

    Q_last, Q_hist, inventory_hist, act_hist, reward_hist, order_hist = train()

    # sub_folder = '{}_{}.{}.{}'.format('bandit', args.thres, datetime.now().strftime('%M'), datetime.now().strftime('%S'))
    sub_folder = '{}_{}'.format('nomem_myopic', temper)
    ...
    with open('{}/act.pickle'.format(log_dir), 'wb') as fp:
        pickle.dump(act_hist, fp)
